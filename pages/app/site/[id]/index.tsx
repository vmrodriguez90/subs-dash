...
export default function SiteIndex() {
  ...
  const { data } = useSWR<SitePlanData>(
    siteId && `/api/plan?siteId=${siteId}&published=true`,
    fetcher,
    {
  ...
  async function createPlan(siteId: string) {
    try {
      const res = await fetch(`/api/plan?siteId=${siteId}`, {
      ...
      <div className="py-20 max-w-screen-xl mx-auto px-10 sm:px-20">
        <div className="flex flex-col sm:flex-row space-y-5 sm:space-y-0 justify-between items-center">
          <h1 className="font-cal text-5xl">
            Plans for {data ? data?.site?.name : "..."}
          </h1>
          <button
            onClick={() => {
              setCreatingPlan(true);
              createPlan(siteId as string);
            }}
            className={`${
              creatingPlan
          ...
          >
            {creatingPlan ? (
              ...
              <>
                New Plan <span className="ml-2">＋</span>
              </>
            )}
          </button>
        </div>
                ...
                <Link href={`/plan/${plan.id}`} key={plan.id}>
                  <div className="flex flex-col md:flex-row md:h-60 rounded-lg overflow-hidden border border-gray-200">
                    <div className="relative w-full h-60 md:h-auto md:w-1/3 md:flex-none">
                      {plan.image ? (
                        <BlurImage
                          alt={plan.title ?? "Unknown Thumbnail"}
                          width={500}
                          height={400}
                    ...
                    <div className="relative p-10">
                      <h2 className="font-cal text-3xl">{plan.title}</h2>
                      <p className="text-base my-5 line-clamp-3">
                        {plan.description}
                      </p>
                      <a
                        className="font-cal px-3 py-1 tracking-wide rounded bg-gray-200 text-gray-600 absolute bottom-5 left-10 whitespace-nowrap"
                        href={`https://${data.site?.subdomain}.vercel.pub/${plan.slug}`}
                        onClick={(e) => e.stopPropagation()}
                        rel="noreferrer"
                        target="_blank"
                      >
                        {data.site?.subdomain}.vercel.pub/{plan.slug} ↗
                      </a>
                    </div>
                  </div>
                </Link>
